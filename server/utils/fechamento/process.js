...
function extrairBaseMetricsShopee(buffer) {
  if (!buffer) return [];
...
function analisarBaseMetrics(baseMetrics) {
  const faturamentoTotal = baseMetrics.reduce((acc, row) => acc + row.faturamento, 0);
  const unidadesTotais = baseMetrics.reduce((acc, row) => acc + row.unidades, 0);
  const pedidosTotais = baseMetrics.reduce((acc, row) => acc + row.pedidos, 0);
...
function compilarFechamentos(buffers) {
  try {
    const lista = Array.isArray(buffers) ? buffers : [];
    if (!lista.length) return { error: "Nenhum arquivo enviado." };

    const merged = new Map();

    for (const buffer of lista) {
      const resultado = processarFechamento(buffer);
      if (!resultado || resultado.error) continue;

      const baseMetrics = extrairBaseMetricsShopee(buffer);

      for (const row of baseMetrics) {
        const key = buildKey(row.idItem, row.produto);
        ...
        acc._ctrW += (row.ctr || 0) * imp;
        acc._convW += (row.conversao || 0) * imp;
        if (!acc.skuReferencia && row.skuReferencia) acc.skuReferencia = row.skuReferencia;
      }
    }

    const baseMetricsCompilado = [...merged.values()].map((row) => {
      const imp = row.impressoes || 0;
      return {
        idItem: row.idItem,
        produto: row.produto,
        ...
        cliques: row.cliques,
        ctr: imp > 0 ? row._ctrW / imp : 0,
        conversao: imp > 0 ? row._convW / imp : 0,
        skuReferencia: row.skuReferencia || "",
      };
    });

    return analisarBaseMetrics(baseMetricsCompilado);
  } catch (error) {
    console.error("ERRO NA API:", error);
    return {
      error: error instanceof Error ? error.message : "Erro ao processar a planilha.",
